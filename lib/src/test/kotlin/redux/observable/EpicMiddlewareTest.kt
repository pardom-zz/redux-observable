    ...
    describe("EpicMiddleware") {
        ...
        describe("create") {
            ...
            it("should accept a epic argument that wires up a stream of actions to a stream of actions") {
                val reducer = { state: List<Any>, action: Any -> state + action }

                val epic = { actions: Observable<out Any>, store: Store<List<Any>> ->
                    Observable.merge(
            ...
            it("should allow you to replace the root epic with replaceEpic") {
                ...
                val epic1 = { actions: Observable<out Any>, store: Store<List<Any>> ->
                    Observable.merge(
                            actions.ofType(Fire1::class.java).map { Action1 },
                            actions.ofType(Fire2::class.java).map { Action2 },
                ...
                val epic2 = { actions: Observable<out Any>, store: Store<List<Any>> ->
                    Observable.merge(
